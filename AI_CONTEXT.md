...
- **Licencia y Modelo**: Licencia temporal Source-Available bajo **PolyForm Noncommercial License 1.0.0** (Licensor / Autor: Luis Alejandro Sosa Camacho). El proyecto cuenta con una transición planificada a una **licencia de código abierto (Open Source)**. Código visible para inspección y uso personal no comercial; redistribución no autorizada por terceros temporalmente restringida.
...
4. **`TimeSchedulerHelper` & `TimeTriggerReceiver`** (`engine/triggers/`): Motor de programación horaria de máxima precisión utilizando `AlarmManager.setAlarmClock` con `AlarmClockInfo`, garantizando la ejecución puntual e inmune a las restricciones de Doze Mode o modos de juego. Reprograma automáticamente las alarmas tras el reinicio del dispositivo (`BootReceiver`) y tras guardar, modificar o eliminar atajos/automatizaciones con su ID persistido (`savedId`).
5. **Permisos de Superposición y Segundo Plano (`SYSTEM_ALERT_WINDOW`)**:
   - `OverlayPermissionHelper` y `OverlayPermissionDialog`: Solicitud guiada al inicio de la app para "Mostrar sobre otras aplicaciones" (`Settings.ACTION_MANAGE_OVERLAY_PERMISSION`), permitiendo que `AppLauncherActionHandler` abra juegos y apps sin ser bloqueado por las restricciones de background activity start de Android 10+.
6. **`AppShortcutsHelper`**: Sincroniza dinámicamente los atajos favoritos con el launcher de Android mediante `ShortcutManagerCompat` para ejecución directa desde el icono de la app.
